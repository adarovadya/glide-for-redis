...
impl StandaloneClient {
    pub async fn create_client(
        ...
        let discover_az = if let Some(ClientReadFrom::AZAffinity(_)) = connection_request.read_from
        {
            true
        } else {
            false
    ...
    async fn round_robin_read_from_replica_az_awareness(
        ...
        loop {
            ...
            let replica_az = replica.get_connection().await.unwrap().get_az();

            // Check if this replica’s AZ matches the user’s AZ.
            if replica_az.as_deref() == Some(client_az.as_str()) {
                println!("------------------------here ---------------------------");
